...
class FolderTree(pulumi.ComponentResource):
    def __init__(self,
        ...
        def build_node(node_name: str, children_spec: Any, parent_id: Optional[pulumi.Input[str]] = None, prefix: str = ""):
            metadata = {}
            children = {}
            ...
            resource_name = f"{tree_name}-{resource_id}".replace("-", "_").replace(" ", "_")
            ...
            for child_name, grandchild_spec in children.items():
                build_node(child_name, grandchild_spec, parent_id=folder.id, prefix=f"{prefix}{node_name}-")
